                            ...
                            <table class="table zero-configuration" id="">
                                ...
                                <tbody>
                                    <tr v-if='users.data.length>0  && !loading' v-for='user in users.data '>
                                        ...
                                        <td>
                                            <select class="form-control form-control-sm select-custom" required id="crmconatct" v-on:change="onSelection(user.user_id,'crmconatct',$event)" name="crmconatct">
                                                <option :selected="user.crmconatct == 'view_only'" value="view_only">View Only</option>
                                                <option :selected="user.crmconatct == 'grant_acces'" value="grant_acces">Grant Access</option>
                                                <option :selected="user.crmconatct == 'authorized'" value="authorized">Authorized</option>
                                                <option :selected="user.crmconatct == 'denied'" value="denied">Denied</option>
                                            </select>
                                        </td>
                                        <td>
    ...
    var vm = new Vue({
        ...
        methods: {
            ...
            getUsers: function(load){
                var self = this;
                $.ajax({
                    url: '{{route("ajaxvolunteers_permissions")}}',
                    type: 'post',
                    data: {
                        _token: '{{csrf_token()}}',
                        search: self.search,
